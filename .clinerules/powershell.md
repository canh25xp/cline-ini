# Rule for wring powershell scripts

## Naming convention

Always use **Approved Verbs** when naming a function:
...
Convert     cv          Data           Changes the data from one representation to another when the cmdlet supports bidirectional conversion or when the cmdlet supports conversion between …
...
Deploy      dp          Lifecycle      Sends an application, website, or solution to a remote target[s] in such a way that a consumer of that solution can access it after deployment is com…
...
Only use `Write-Host`
Use `Write-Debug` for unimportant information.
Use `Write-Error` to output error message.

## Comments

Don't write redundant comments if the function or variable name is already self described.

Here's a BAD comments example
...
Here's a GOOD comments example
